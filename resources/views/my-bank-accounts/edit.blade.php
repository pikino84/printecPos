...
<div class="card">
    <div class="card-header">
        <h6>Editar Cuenta: {{ $bankAccount->bank_name }}</h6>
        <small class="text-muted">{{ $bankAccount->alias }}</small>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('my-bank-accounts.update', $bankAccount->id) }}">
            @csrf
            @method('PUT')
                    ...
                    <select name="partner_entity_id" class="form-control @error('partner_entity_id') is-invalid @enderror" required>
                        ...
                        @foreach($entities as $entity)
                            <option value="{{ $entity->id }}" 
                                {{ old('partner_entity_id', $bankAccount->partner_entity_id) == $entity->id ? 'selected' : '' }}>
                                {{ $entity->razon_social }}
                                @if($entity->rfc)
                                    - RFC: {{ $entity->rfc }}
                                @endif
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('clabe', $bankAccount->clabe) }}"
                               maxlength="18"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('swift', $bankAccount->swift) }}"
                               maxlength="20"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="currency" class="form-control @error('currency') is-invalid @enderror">
                            <option value="MXN" {{ old('currency', $bankAccount->currency) == 'MXN' ? 'selected' : '' }}>MXN - Peso Mexicano</option>
                            <option value="USD" {{ old('currency', $bankAccount->currency) == 'USD' ? 'selected' : '' }}>USD - Dólar Americano</option>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="feather icon-save"></i> Actualizar Cuenta
                </button>
